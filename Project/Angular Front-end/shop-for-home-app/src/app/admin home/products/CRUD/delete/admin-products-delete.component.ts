...
@Component({
    selector: "admin-products-delete-component",
    templateUrl: "admin-products-delete.component.html",
    standalone: true,
    imports: [RouterLink, RouterOutlet, RouterLinkActive, FormsModule]
})
export class AdminProductsDeleteComponent{
    ...
    deleteUser(){
        if (this.productId) {
            this.client.productsDelete(this.productId).pipe(
                catchError((error) => {
            ...
            this.message = "Enter Product id to delete record!";
            let action = "<span class='text-danger'>Error</span>";
            ActionToast.showToast(ActionToast.apps.productsApp, action, this.message, ActionToast.action.errorColor);
        }
